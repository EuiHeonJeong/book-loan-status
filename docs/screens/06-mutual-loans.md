...
- 탭 바 아래 접기/펼치기 **가족 필터**(체크박스, 기본값 전체 체크) — 두 탭 공통으로 적용된다. 일반예약현황과 달리 **도서관 필터는 넣지 않는다**(소장도서관/수령처 두 필드가 같이 있어 "도서관"이라는 단일 필터 축이 모호해서 사용자 요청으로 제외, 2026-08-27).
...
- `MutualLoansPage`
  - `AppBar` (title, back)
  - 탭 바 (신청현황/이력현황)
  - `FilterPanel` (접기/펼치기, `FamilyCheckboxGroup`만 — 도서관 필터 없음)
...
- `filterOpen: boolean`, `family: Record<memberName, boolean>` — 두 탭 공통으로 적용되는 클라이언트 필터
